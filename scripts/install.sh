...
# Shutdown service: must run ExecStop BEFORE Yakuake's autostart unit stops,
# otherwise Yakuake is already dead and we can't query session state.
# systemd reverses After=/PartOf= for stops, so After=<yakuake unit> means
# we stop before it.
#
...
[Install]
WantedBy=$YAKUAKE_AUTOSTART_UNIT
EOF
